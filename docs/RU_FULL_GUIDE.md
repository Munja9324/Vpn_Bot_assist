# Полная инструкция по установке и настройке VPN_KBR_BOT userbot

Документация написана для проекта `VPN_KBR_BOT userbot`. Это не обычный Bot API бот, а userbot: программа входит в Telegram под твоим личным аккаунтом и через Telethon работает с админ-ботом VPN-сервиса.
